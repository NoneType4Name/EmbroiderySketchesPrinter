...
class SceneMain:
    def __init__(self, parent):
        ...
        self.ruler_x = pygame.surface.Surface((self.monitor.mmTOpx(self.monitor.width_mm, 10*2)), pygame.SRCALPHA)
        self.ruler_y = pygame.surface.Surface((self.monitor.mmTOpx(10*2, self.monitor.height_mm)), pygame.SRCALPHA)
        threading.Thread(target=self._draw_ruler_x, daemon=True).start()
        threading.Thread(target=self._draw_ruler_y, daemon=True).start()
    ...
    def _draw_ruler_x(self):
        for num in numpy.arange(0, self.monitor.width_mm/10-2, 0.5):
            pygame.draw.line(self.ruler_x, (0, 0, 0), self.monitor.mmTOpx(num*10, 0), self.monitor.mmTOpx(num*10, 10 if int(num)==num else 7), self.monitor.mmTOpx(0.2))
            if num == int(num):
                num_img = GUI.Font.render(str(round(num)), pygame.Rect(0, 0, *self.monitor.mmTOpx(5, 5)), True, (0, 0, 0))
                self.ruler_x.blit(num_img, (self.monitor.mmTOpx(num*10)-num_img.get_size()[0]/2, self.monitor.mmTOpx(10)))
    
    def _draw_ruler_y(self):
        for num in numpy.arange(0, self.monitor.height_mm/10-1-2, 0.5):
            pygame.draw.line(self.ruler_y, (0, 0, 0), self.monitor.mmTOpx(10 if int(num)==num else 13, num*10), self.monitor.mmTOpx(20, num*10), self.monitor.mmTOpx(0.2))
            if num == int(num):
                num_img = pygame.transform.rotate(GUI.Font.render(str(round(num)), pygame.Rect(0, 0, *self.monitor.mmTOpx(5, 5)), True, (0, 0, 0)), 270)
                self.ruler_y.blit(num_img, (self.monitor.mmTOpx(10-num_img.get_size()[1]/(4 if num // 10 else 2)), self.monitor.mmTOpx(num*10)-num_img.get_size()[0]/(2 if num // 10 else 4)))
    ...
    def update(self):
        ...
        if self.sketch_zoom == 1:
            ...
            self.image.blit(self.ruler_y, (self.size[0]-self.monitor.mmTOpx(20), 0))
        ...
        if self.notifications.sprites():
            ...
            if self.updated_data:
                ...
                self.updated_data = False
            if self.sketch_zoom_precise_y_t and time.time()-self.sketch_zoom_precise_y_t >= 1:
                self.sketch_zoom_precise_y_t = 0
                threading.Thread(target=self.UpdateZoom, daemon=True, args=[Image.LANCZOS]).start()
            events = tuple(map(lambda e: e if (e.type == pygame.KEYDOWN and ((e.key in (pygame.K_RIGHT, pygame.K_LEFT, pygame.K_UP, pygame.K_DOWN)) or ((e.key in (pygame.K_EQUALS, pygame.K_MINUS)) and pygame.key.get_mods() & pygame.KMOD_LCTRL))) else False, self.parent.events))
            ...
            for event in (e for e in events if e):
                if event.key in (pygame.K_RIGHT, pygame.K_LEFT, pygame.K_UP, pygame.K_DOWN):
                    ...
                    if event.key == pygame.K_EQUALS and self.sketch_zoom + 0.01 <= 2:
                        self.sketch_zoom += 0.01
                    elif event.key == pygame.K_MINUS and self.sketch_zoom - 0.01 > 0:
                        self.sketch_zoom -= 0.01
                    else:
                        continue
                    self.sketch_zoom_precise_y_t = time.time()
                    threading.Thread(target=self.UpdateZoom, daemon=True, args=[Image.NEAREST]).start()
            if self.data_panel.allowed_sketches and any(map(lambda e: ((e.type == pygame.MOUSEMOTION and e.buttons[0]) or (e.type == pygame.MOUSEWHEEL and 0 < self.sketch_zoom + e.precise_y/10)), self.parent.events)) and ((not self.data_panel.isCollide()) if self.data_panel.open else True):
                for event in self.parent.events:
                    if event.type == pygame.MOUSEMOTION:
                        ...
                        if 0 < self.sketch_zoom + event.precise_y/10 <= 2:
    ...
    def SketchCrop(self):
        cropt = self._sketch_zoomed.crop((*-numpy.array(self.sketch_pos), *tuple(-numpy.array(self.sketch_pos)+(self.size))))
